...
#This code works at the moment, do not change the logic, just add the forces
...
def attempt_flip(grid, persist):
    ...
    for idx, neighbor_set in enumerate(neighbors_outer):
            ...
            if (x, y) in neighbor_set:
                ...
                if neighbor_types.issubset({CELL_TYPE[1]}):
                    ...
                    if not is_attached_to_main_body(grid, (x, y), CELL_TYPE[1], min_size=1):
                        grid[x, y] = old_type
                ...
                #If surrounded by ECM
                    ...
                    grid[x, y] = CELL_TYPE[4]
                    grid[initial_out_pos] = CELL_TYPE[0]
                    Filaments[idx] = ((x, y),Filaments[idx][1])
                    swapped = True
                    #Check cyctoplasm attachment
                    if not is_attached_to_main_body(grid, (x, y), CELL_TYPE[1], min_size=1):
                        grid[x, y] = old_type
                        grid[initial_out_pos] = CELL_TYPE[4]
                        Filaments[idx] = (initial_out_pos, Filaments[idx][1])
        ...
        for idx, neighbor_set in enumerate(neighbors_inner):
            ...
            if (x, y) in neighbor_set:
                ...
                if neighbor_types.issubset({CELL_TYPE[1]}):
                    ...
                    if not is_attached_to_main_body(grid, (x, y), CELL_TYPE[2], min_size=1):
                        grid[x, y] = old_type
    ...
    # ===========================
    #If no swap has occured and the intial point is not a filament
